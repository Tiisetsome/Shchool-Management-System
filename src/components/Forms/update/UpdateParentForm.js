...
const UpdateParentForm = () => {
    ...
    // Destrcuture items
    const {parents, updatePerson, addStatus}  = adminContext
    
    // Get id
    const {id} = useParams();

    // Get referred student
    const [currentParent] = parents.filter(parent => parent.parent_id === id);
    console.log(currentParent);

    // Form state
    const [fields, setFields] = useState({
        id: currentParent.id,
        fname: currentParent.fname,
        lname: currentParent.lname,
