...
const UpdateTeacherForm = () => {
    ...
    // Destrcuture items
    const {updatePerson, addStatus, teachers}  = adminContext

    // Get id
    const {id} = useParams();

    // Get referred student
    const [currentTeacher] = teachers.filter(teacher => teacher.teacher_id === id);
...
export default withRouter(UpdateTeacherForm);
